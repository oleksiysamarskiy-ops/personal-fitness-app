...
const Icons = {
  ...
  Droplet: () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{width:16,height:16}}><path d="M12 2.69l5.66 5.66a8 8 0 1 1-11.31 0z"/></svg>,
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .fade-in   { animation: fadeUp 0.28s cubic-bezier(.22,.9,.42,1) both; }
  .fade-up   { animation: fadeUp 0.28s cubic-bezier(.22,.9,.42,1) both; }
  .slide-up  { animation: slideSheet 0.32s cubic-bezier(.22,.9,.42,1) both; }
  .slide-sh  { animation: slideSheet 0.32s cubic-bezier(.22,.9,.42,1) both; }
  .pop-in    { animation: popIn 0.22s cubic-bezier(.22,.9,.42,1) both; }
  .check-pop { animation: popIn 0.22s cubic-bezier(.22,.9,.42,1) both; }
  ...
  .water-fill { animation: waterFill 0.6s cubic-bezier(.22,.9,.42,1) both; }
`;
...
function Modal({ open, onClose, title, children }) {
  ...
  return (
    <div
      style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.65)", backdropFilter:"blur(4px)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:1000 }}
      onClick={e => e.target === e.currentTarget && onClose()}
    >
      <div className="slide-up" style={{ background:"var(--bg2)", borderRadius:"20px 20px 0 0", width:"100%", maxWidth:500, maxHeight:"92vh", overflowY:"auto", paddingBottom:"env(safe-area-inset-bottom,20px)" }}>
        ...
        <div style={{ padding:"16px 20px 0", display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
          ...
          <button onClick={onClose} style={{ background:"var(--card2)", border:"none", borderRadius:8, padding:"6px 8px", color:"var(--text2)", display:"flex", lineHeight:0, cursor:"pointer" }}><Icons.X/></button>
        </div>
...
// ─── БЛОК ВОДЫ (переработан) ──────────────────────────────────────────────────

function WaterBlock({ waterData, setWaterData, waterGoal }) {
  ...
  const done = todayWater.total >= waterGoal;
  ...
  // Визуальные сегменты стакана (8 порций по waterGoal/8)
  const segments = 8;
  const filledSegments = Math.min(segments, Math.round((todayWater.total / waterGoal) * segments));
    ...
    <Card style={{ padding:"16px", overflow:"hidden" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14 }}>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <div style={{ width:32, height:32, borderRadius:10, background:"var(--blue-bg)", display:"flex", alignItems:"center", justifyContent:"center" }}>
            <span style={{ color:"var(--blue)", display:"flex" }}><Icons.Droplet/></span>
          </div>
          <div>
            <p style={{ fontWeight:700, fontSize:14 }}>Вода</p>
            <p style={{ color:"var(--text3)", fontSize:11, marginTop:1 }}>Цель: {waterGoal} мл</p>
          </div>
        </div>
        <div style={{ textAlign:"right" }}>
          <p style={{ fontWeight:800, fontSize:22, letterSpacing:"-0.5px", color: done ? "var(--green)" : "var(--blue)" }}>
            {todayWater.total}
            <span style={{ fontSize:13, fontWeight:500, color:"var(--text3)" }}> мл</span>
          </p>
          {!done && <p style={{ fontSize:11, color:"var(--text3)", marginTop:1 }}>ещё {remaining} мл</p>}
          {done && <p style={{ fontSize:11, color:"var(--green)", marginTop:1, fontWeight:700 }}>Цель достигнута! 🎉</p>}
        </div>
      </div>

      {/* Волнообразный прогресс */}
      <div style={{ position:"relative", height:10, background:"var(--bg3)", borderRadius:10, overflow:"hidden", marginBottom:14 }}>
        <div
          ...
          style={{
            ...
            width:`${pct}%`,
            background: done
              ? "linear-gradient(90deg, var(--green), #5ce3a7)"
              : "linear-gradient(90deg, #3b82f6, var(--blue))",
            borderRadius:10,
            transition:"width 0.5s cubic-bezier(.22,.9,.42,1)",
            boxShadow: done ? "0 0 12px rgba(62,207,142,0.4)" : "0 0 12px rgba(96,165,250,0.3)",
          }}
        />
        {/* Пузырьки — декоративные точки */}
        {pct > 8 && <div style={{ position:"absolute", top:"50%", left:`${Math.min(pct-6,90)}%`, transform:"translateY(-50%)", width:4, height:4, borderRadius:"50%", background:"rgba(255,255,255,0.4)" }}/>}
        {pct > 20 && <div style={{ position:"absolute", top:"50%", left:`${Math.min(pct-14,80)}%`, transform:"translateY(-50%)", width:3, height:3, borderRadius:"50%", background:"rgba(255,255,255,0.3)" }}/>}
      </div>
        ...
        {[150, 250, 500].map(ml => (
          <button
            ...
            style={{
              ...
              color:"var(--blue)", fontWeight:700, fontSize:13,
      ...
      <div style={{ display:"flex", gap:8, marginBottom:10 }}>
        ...
        <button
          ...
          style={{
            ...
            cursor:"pointer", fontFamily:"var(--font)", fontSize:14, flexShrink:0,
          }}
        >+</button>
      </div>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        ...
        >
          ...
          {todayWater.log.length} {todayWater.log.length === 1 ? "запись" : todayWater.log.length < 5 ? "записи" : "записей"}
        </button>
        {todayWater.log.length > 0 && (
          <button
            onClick={removeLastWater}
            style={{
              background:"var(--red-bg)", border:"none", borderRadius:7,
              padding:"4px 10px", color:"var(--red)", fontSize:12,
      ...
      {showLog && todayWater.log.length > 0 && (
        <div className="expand-down" style={{ borderTop:"1px solid var(--border)", marginTop:10, paddingTop:10, display:"flex", flexDirection:"column", gap:5 }}>
          {[...todayWater.log].reverse().map((entry, i) => (
            <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <span style={{ color:"var(--text3)", fontSize:12 }}>{entry.time}</span>
              <span style={{ color:"var(--blue)", fontWeight:700, fontSize:13 }}>+{entry.amount} мл</span>
            </div>
          ))}
        </div>
      )}
...
function TodayPage({ routine, weightHistory, nutrition, workouts, completions, setCompletions, goals, waterGoal, waterData, setWaterData }) {
  ...
  function taskContent(task) {
    if (task.type === "meal") {
      ...
      return {
        subtitle: `${cal} ккал`,
        detail: meal?.items?.map(i => `${i.name} — ${i.grams}г (${Math.round(i.grams*(i.calories??i.caloriesPer100g??0)/100)} ккал)`).join("\n") || "Нет данных"
      };
    }
  ...
  return (
    <div style={{ padding:"0 0 calc(var(--nav-h) + 20px)" }}>
        ...
        <Card style={{ padding:"14px 16px" }}>
          ...
          {diff !== null && (
            <div style={{ display:"flex", alignItems:"center", gap:4, marginTop:8 }}>
              ...
              <span style={{ fontSize:13, fontWeight:700, color: parseFloat(diff) < 0 ? "var(--green)" : parseFloat(diff) > 0 ? "var(--red)" : "var(--amber)" }}>
        ...
        <Card style={{ padding:"16px" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10 }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:6 }}>
              ...
              {goals.calories > 0 && <span style={{ fontSize:13, color:"var(--text3)" }}>/ {goals.calories}</span>}
      ...
      {/* ─ Вода (новый компонент) ─ */}
      <div style={{ padding:"10px 16px 0" }}>
        <WaterBlock waterData={waterData} setWaterData={setWaterData} waterGoal={waterGoal || 2500} />
      </div>
            ...
            return (
              ...
              >
                <div style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 14px" }}>
                  ...
                  >
                    {done && <span className="check-pop" style={{color:"#0a0a0f", display:"flex"}}><Icons.Check/></span>}
                  </button>
          ...
          return (
            <Card key={mt} style={{ overflow:"hidden" }}>
              ...
              >
                ...
                <Tag color={cfg.color}>{macros.calories} ккал</Tag>
          ...
          <Card style={{ overflow:"hidden" }}>
            <div style={{ padding:"16px" }}>
              <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between" }}>
                <div>
                  <Tag color="accent">Тренировка</Tag>
              ...
              <p style={{ color:"var(--text3)", fontSize:12, marginTop:8, fontWeight:600 }}>{workout.exercises.length} УПРАЖНЕНИЙ</p>
            </div>
